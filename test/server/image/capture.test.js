...
import { isImageFile, loadImageFromFile } from '../../../server/image/capture.js'
...
describe('loadImageFromFile', () => {
  const okPath = join(tmpdir(), `img-annotator-ok-${process.pid}.png`)
  const badExtPath = join(tmpdir(), `img-annotator-bad-${process.pid}.gif`)
  const oversizedPath = join(tmpdir(), `img-annotator-big-${process.pid}.png`)
...
// Append to test/server/capture.test.js
...
describe('isSupportedCaptureUrl', () => {
  it('accepts http and https', async () => {
    const { isSupportedCaptureUrl } = await import('../../../server/image/capture.js')
...
describe('captureUrl', () => {
  it('returns the screenshot buffer and page dimensions', async () => {
    const { captureUrl } = await import('../../../server/image/capture.js')
